...
class cprint:
	...
	def __init__(self, value='cprint'):
	...
	def debug_p(self, string, mode='defult', fore='green', back=''):
		...
		print ("%s%s [%s line:%s] %s%s" % (style, datetime.datetime.now(), __file__, f.f_lineno, self.name + '-debug:' + string, end))
	...
	def error_p(self, string, mode='defult', fore='red', back=''):
		...
		try:
		    ...
		    f = sys.exc_info()[2].tb_frame.f_back
		print ("%s%s [%s line:%s] %s%s" % (style, datetime.datetime.now(), repr(os.path.abspath(sys.argv[0])), f.f_lineno, self.name + '-error:' + string, end))
